...
class Treino extends BaseController
{
    ...
    public function pesquisar()
    {
        ...
        tr.id AS tipo_id
        ');
        $this->model->from('cad_treino ct');
        $this->model->join('execicio e', 'ct.exer_id = e.id');
        $this->model->join('tipo_grupo tg', 'ct.tipo_grupo_id = tg.id');
        $this->model->join('treino_grupo g', 'tg.grupo_id = g.id');
        $this->model->join('treino_tipo tr', 'tg.treino_id = tr.id');
        $this->model->join('funcionarios f', 'tg.funcionario_id = f.id');
        $this->model->join('cliente c', 'tg.cliente_id = c.id');
        $this->model->where('c.id', $data['cliente_id']);
        $this->model->groupBy('ct.id');
